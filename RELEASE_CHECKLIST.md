...
- [x] Canonical production source prepared for `go2max/cash-for-cars`
- [x] CI source, security, Node syntax, and PHP syntax checks

## Deployment Gates

- [ ] Choose and configure exactly one backend runtime
- [ ] Generate and commit `server/package-lock.json` on a host with approved npm registry access when deploying with Node
